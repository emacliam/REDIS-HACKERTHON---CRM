                    ...
                    <table class="min-w-full table-auto">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            >
                                <td
                                    key=""
                                    class="w-1/4 px-6 py-3 text-sm font-medium text-gray-900 max-w-0 whitespace-nowrap"
                                >
                                    <div
...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            socket.on('change-issue-status-response', async (response) => {
                //TODO: CHECK RESPONSE STATUS
                console.log('issue status changed')
                console.log(response.data)
                const newIssue = store.getters['issue/Issue']
                newIssue.issue_status = response.data.new_issue_status
                await store.dispatch('issue/Save_Issue', newIssue)
                issues.value = issues.value.filter(
                    (issue) => issue.pk !== response.data.pk
                )
            })
        ...
        async function FETCH_DASHBOARD() {
            try {
                loading.value = true
                const response = await DashboardService.getAllAgent()
        ...
        async function CHANGE_ISSUE_STATUS(id, status, status1) {
            try {
                const data = {
                    issue_id: id,
        ...
        async function JOIN_ROOM(id, issue) {
            ...
            try {
                await CHANGE_ISSUE_STATUS(id, 'PROGRESS', issue.issue_status)
                const data = {
                    issue_id: id,
        ...
        return {
            ...
            pinnedProjects,
            PROGRESS: computed(() =>
                issues.value.filter((iss) => iss.issue_status === 'PROGRESS')
            ),
            sidebarOpen,
            NavigateToChat,
            dashboard,
